...
final readonly class UserRepository
{
    ...
    {
        $stmt = $this->pdo->prepare('INSERT INTO users (email, password_hash, created_at) VALUES (:email, :password_hash, :created_at)');
        $stmt->execute([
            'email' => $email,
    ...
    {
        $stmt = $this->pdo->prepare('INSERT INTO sessions (user_id, token, created_at, expires_at) VALUES (:user_id, :token, :created_at, :expires_at)');
        $stmt->execute([
            'user_id' => $userId,
            'token' => $token,
            'created_at' => date('Y-m-d H:i:s'),
